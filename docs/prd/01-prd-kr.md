...
| Skills | 2 | explain, refactor |
| PAUL.md | 1 | 플러그인 지시문 |
| Templates | 4 | coding-style, git-workflow, testing, security |
...
사용자가 "이 코드 설명해줘" → explain 스킬 활성화 → 구조화된 코드 설명 제공

### 시나리오 6: 리팩토링 (Skill)

사용자가 "이 함수를 리팩토링해줘" → refactor 스킬 활성화 → 분석 → 계획 → 사용자 확인 → 실행
...
- [ ] explain 스킬: 코드 설명 요청 시 구조화된 5단계 설명 제공
- [ ] refactor 스킬: 리팩토링 시 Plan → Confirm → Execute → Verify 흐름
- [ ] PAUL.md가 system prompt에 주입되어 LLM이 oh-my-paul 기능을 인식
- [ ] templates/rules/ 파일을 프로젝트에 복사하여 사용 가능
